...
export default function BlogListView({ posts }: { posts: BlogCard[] }) {
  ...
  return (
    <div>
        ...
        <div className="px-5 pb-6 md:px-10">
          <Link href={`/blog/${featured.slug}/`} className="group block">
            ...
            <div className="pt-3.5">
              ...
              <h2 className="m-0 font-heading text-xl font-semibold leading-snug text-charcoal">{featured.title}</h2>
              {featured.excerpt && <p className="mt-2 text-[13px] leading-relaxed text-[#888]">{featured.excerpt}</p>}
      ...
      {/* Rest as a grid/list */}
      {rest.length > 0 && (
        <div className="grid gap-6 px-5 py-6 md:grid-cols-2 md:px-10 lg:grid-cols-3">
          {rest.map((post) => (
            <Link key={post.id} href={`/blog/${post.slug}/`} className="group flex gap-3.5 md:flex-col md:gap-0">
              <div className="h-[110px] w-[110px] flex-shrink-0 overflow-hidden bg-cream md:h-auto md:w-full md:aspect-[16/10]">
                {post.featured_image && (
                  // eslint-disable-next-line @next/next/no-img-element
              ...
              <div className="flex flex-1 flex-col justify-center md:pt-3">
