...
/**
 * Shared blog-post body, used by BOTH the public cached route and the admin
 * preview route. No dynamic APIs here so the public route stays cached.
 */
export default async function BlogPostContent({ slug, preview = false }: { slug: string; preview?: boolean }) {
  const p = await getBlogPost(slug, preview);
  if (!p) notFound();
