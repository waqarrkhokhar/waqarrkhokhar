...
export async function generateMetadata({ params }: { params: { path: string[] } }): Promise<Metadata> {
  ...
  const image = page.banner_image || (await ogDefaultImage());
  ...
  return { title, description, robots: robotsMeta(page.robots), alternates: { canonical }, openGraph: buildOg({ path: canonical, title, description, image, type: "website" }) };
}
