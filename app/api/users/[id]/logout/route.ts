...
export async function POST(_req: Request, { params }: Params) {
  ...
  const { data, error } = await (admin as unknown as { rpc: (fn: string, args: Record<string, unknown>) => Promise<{ data: unknown; error: { message: string } | null }> }).rpc("admin_revoke_user_sessions", { target: params.id });
